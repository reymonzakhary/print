...
class AddLayerAction extends Action implements ActionContractInterface
{
    ...
    public function handle(): void
    {
        ...
        $result = app(PdftoolService::class)->addLayer(
            layers: $assets,
    ...
    )
    {
        return collect($this->uses)->map(fn($uses) => match ($uses) {
            'assets' => $this->loadAssets($this->pipeline->{$uses}, $row, $cloud)
        })->first();
    }
    ...
    ): array
    {
        ...
        collect($collection)->each(function ($obj) use (&$assets, $row, $cloud) {
