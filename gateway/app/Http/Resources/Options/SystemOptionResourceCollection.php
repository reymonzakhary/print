...
class SystemOptionResourceCollection extends ResourceCollection
{
    /**
     * @var array
     */
    ...
    #[NoReturn] final protected function processCollection($request): array
    {
        ...
        return $this->collection->filter(fn($collection, $key) => $key === 'data')->map(fn($resource) => collect($resource->resource)->map(function ($resource) use ($request) {
            $resource = new SystemOptionResource($resource);
            return $resource->hide($this->withoutFields)->toArray($request);
        }))->put('links', $this->preparePagination($request))->put('meta', $this->prepareMeta($request))->all();
    }
