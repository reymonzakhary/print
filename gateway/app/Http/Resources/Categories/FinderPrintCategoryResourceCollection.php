...
class FinderPrintCategoryResourceCollection extends ResourceCollection
{
    /**
     * @var array
     */
    ...
    final protected function processCollection($request): array
    {
        ...
        return $this->collection->filter(fn($collection, $key) => $key === 'data')->map(fn($resource) => collect($resource->resource)->map(function ($resource) use ($request) {
            $resource = new FinderPrintCategoryResource($resource);
     ...
     * @return mixed
     */
    protected function preparePaginatedObject($request): mixed
    {
