...
class PrintSupplierCategoryResourceCollection extends ResourceCollection
{
    /**
     * @var array
     */
    ...
    final protected function processCollection($request): array
    {
        ...
        return $this->collection->filter(fn($collection, $key) => $key === 'data')->map(fn($resource) => collect($resource->resource)->map(function ($resource) use ($request) {
            $resource = new PrintSupplierCategoryResource($resource);
    ...
    protected function praperMeta($request): array
    {
        ...
        return [
            ...
            'links' => [
                [
                    'url' => $this->pagination['prev_page_url'] ? "{$request->url()}{$this->pagination['prev_page_url']}" : null,
                    'label' => "&laquo; Previous",
                    'active' => (bool)$this->pagination['prev_page_url'],
                ],
                [
                    'url' => $this->pagination['current_page'] ? "{$request->url()}?page={$this->pagination['current_page']}" : null,
                    "label" => "Current page",
                    "active" => true
                ],
                [
                    'url' => $this->pagination['next_page_url'] ? "{$request->url()}{$this->pagination['next_page_url']}" : null,
                    "label" => "Next &raquo;",
