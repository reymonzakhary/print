...
class SupplierCatalogueResourceCollection extends ResourceCollection
{
    /**
     * @var array
     */
    ...
    final protected function processCollection($request): array
    {
        ...
        return $this->collection->filter(fn($collection, $key) => $key === 'data')->map(fn($resource) => collect($resource->resource)->map(function ($resource) use ($request) {
            $resource = new SupplierCatalogueResource($resource);
            return $resource->hide($this->withoutFields)->toArray($request);
        }))->put('links', $this->paperPagination($request))->put('meta', $this->paperMeta($request))->all();
    }
     ...
     * @return array
     */
    protected function paperPagination($request): array
    {
        if (!$this->pagination) {
            return [];
        }
     ...
     * @return array
     */
    protected function paperMeta($request): array
    {
        if (!$this->pagination) {
            return [];
        }
