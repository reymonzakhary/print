...
namespace App\Http\Controllers\Tenant\Mgr\Custom\Products\Stocks;
...
class ProductStocksController extends Controller
{
     ...
     *     path="/api/v1/mgr/custom/products/{product}/stocks",
     *     summary="Get Product with Stock",
     *     security={{ "Bearer":{} }},
     *     @OA\Response(
     *     response="200",
     ...
     *     @OA\JsonContent(
     *          @OA\Property(type="array", property="data", @OA\Items(ref="#/components/schemas/StockResource")),
    ...
    {
        return StockResource::collection($product->skus()->with([
            'stocks',
            'stocks.location',
        ])->get());
    }
    ...
    /**
     * @param StoreProductsStockRequest $request
     * @return StockResource
     *
     * @OA\Post (
     *     tags={"Custom_Products"},
     *     path="/api/v1/mgr/custom/products/{product}/stocks",
     *     summary="Create Products with Stock",
     *     security={{ "Bearer":{} }},
     *   @OA\RequestBody(
     *      required=true,
     *      description="products setting data",
     *      @OA\JsonContent(ref="#/components/schemas/StoreProductsStockRequest"),
     *   ),
     *     @OA\Response(
     *     response="200", description="success",
     *     @OA\JsonContent(
     *
     * @OA\Property(type="string", title="message", description="message", property="message", example="Stock has been created successfully."),
     * @OA\Property(type="string", title="status", description="status", property="status", example="200"),
     *      )),
     *     @OA\Response(response=401, description="We can't handle this request. please try again later!"),
     * )
     *
     */
    public function store(Product $product, StoreProductsStockRequest $request)
    {
        if ($sku = $product->sku()->first()) {
