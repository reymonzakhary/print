...
/**
 * Class UpdateCustomOptionRequest
 * @package App\Http\Resources\Options
 * @OA\Schema(
 *     schema="UpdateCustomOptionRequest",
 *     title="Custom Update Options Request"
 *
 * )
 */
class UpdateOptionRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'name' => [
                'string', Rule::unique('options')->where(function ($q) {
                    return $q->where([
                        'name' => $this->name,
                        'box_id' => $this->box_id,
                        'iso' => $this->iso
    ...
    {
        if ($this->parent_id === $this->option->row_id ||
            $this->option->children->whereIn('row_id', $this->parent_id)->count() > 0) {
            throw ValidationException::withMessages(['parent_id' =>
                __("The selected parentProd id is invalid, trying to connect to it self or child option.")
            ]);
        }
        ...
        $this->merge(array_merge([
            ...
            'box_id' => $this->box_id ?? $this->option->box_id,
            'base_id' => $this->parent_id ?: $this->option->row_id,
        ], $slug, $translation, $user->toArray()));
    }
}
