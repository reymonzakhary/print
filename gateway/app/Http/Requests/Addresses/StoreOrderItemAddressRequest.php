...
class StoreOrderItemAddressRequest extends FormRequest
{
    /**
    ...
    {
        if(
            $this->order->delivery_multiple &&
            $this->order->items->where('id', $this->item->id)->first()->pivot->delivery_pickup
        ) {
            if(!$this->address) {
        ...
        } elseif ($user = $this->order?->orderedBy) {
            if ($this->address) {
