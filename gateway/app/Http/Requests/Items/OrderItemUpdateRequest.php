...
use App\Validators\PrepareOrderItemValidator;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\ValidationException;

final class OrderItemUpdateRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            ...
            'price.qty' => 'nullable|integer|min:0',
    ...
    public function prepareForValidation(): void
    {
        ...
        if (!auth()->user()->can('orders-items-note-update') && $this->has('note')) {
            throw ValidationException::withMessages([
                'orders_items_note' => __('Not permitted action.')
            ]);
        }

        if (!auth()->user()->can('orders-items-reference-update') && $this->has('reference')) {
            throw ValidationException::withMessages([
                'orders-items-reference' => __('Not permitted action.')
            ]);
        }

        if (!auth()->user()->can('orders-items-product-update') && !empty($this->product)) {
            throw ValidationException::withMessages([
                'orders-items-product' => __('Not permitted action.')
            ]);
        }

        if (!auth()->user()->can('orders-items-product-delivery-days-update') && !empty(
            optional(
                optional(optional($this->product)['prices'])['tables']
            )['dlv']
            )) {
            throw ValidationException::withMessages([
                'orders-items-product-prices' => __('Not permitted action.')
            ]);
        }

        if (!auth()->user()->can('orders-items-product-prices-update') && !empty(
            optional(
                optional(optional($this->product)['prices'])['tables']
            )['p']
            )) {
        ...
        if (!auth()->user()->can('orders-items-delivery-separated-update') && $this->has('delivery_separated')) {
            throw ValidationException::withMessages([
                'delivery_separated' => __('Not permitted action.')
            ]);
        }

        if (!auth()->user()->can('orders-items-st-update') && $this->has('st')) {
            throw ValidationException::withMessages([
                'st' => __('Not permitted action.')
            ]);
        }

        if (!auth()->user()->can('orders-items-st-message-update') && $this->has('st_message')) {
            throw ValidationException::withMessages([
                'st_message' => __('Not permitted action.')
            ]);
        }

        $this->replace((new PrepareOrderItemValidator($this))->prepare());
    }
