...
use Alexusmai\LaravelFileManager\Services\TransferService\Transfer;
...
{
    // files
    foreach ($this->clipboard['files'] as $k => $file) {
        Storage::disk($this->disk)->copy(
            $file,
            $this->renamePath( $this->clipboard['to_files'][$k], $this->path)
        );
    }
