...
use App\Plugins\Util\Groot\DataMapper;
use App\Plugins\Util\Groot\ResponseValidator;
use GuzzleHttp\Exception\GuzzleException;

final class GrootPluginManager extends PluginManager
{
    ...
    ): array
    {
        ...
        $options = $this->dataMapper->mapProductPrindustryToGroot(
            $quantity,
            $productData,
            $categoryData
        );
        ...
        $response = $this->makeRequest('POST', 'get-price',
            formParams: [
                ...
                'article_number' => data_get($categoryData, 'source_slug'),
                'options' => $options,
                'quantity' => $quantity,
            ],
        ...
        try {
            ...
            Log::debug('Failed to retrieve price from Groot', [
                'quantity' => $quantity,
                'response' => $response,
                'error' => $e->getMessage(),
        ...
        return $this->dataMapper->mapPriceFromGrootToPrindustry(
            $quantity,
