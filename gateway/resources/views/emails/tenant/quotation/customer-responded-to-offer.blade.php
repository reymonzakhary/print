            ...
            <table class="content" width="100%" cellpadding="0" cellspacing="0" role="presentation"
                ...
                <tr>
                    <td class="body" width="100%" cellpadding="0" cellspacing="0"
                        style="text-align: center ;box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; position: relative; -premailer-cellpadding: 0; -premailer-cellspacing: 0; -premailer-width: 100%; background-color: #edf2f7; border-bottom: 1px solid #edf2f7; border-top: 1px solid #edf2f7; margin: 0; padding: 0; width: 100%;">
                        <h4>{{__('mails.quotation_response')}}</h4>

                        @php
                            $quotationRelativePath = sprintf('manager/%s/%s', ($isAcceptation ? 'orders' : 'quotations'), $quotation->getAttribute('id'));
                            $quotationFullUrl = $app->make('url')->to($quotationRelativePath);

                            $replacements = [
                                'quotationId' => $quotation->id,
                                'customerEmail' => $quotation->orderedBy->email
                            ];
                        @endphp

                        @if($isAcceptation)
                            {{__('mails.quotation_offer_accepted', replace: $replacements)}}
                        @else
                            {{__('mails.quotation_offer_rejected', replace: $replacements)}}
                        @endif

                        <br><br>
                        <a href="{{$quotationFullUrl}}">{{__('mails.quotation_offer_details_link')}}</a>
                    </td>
                </tr>
                <tr>
                    <td
