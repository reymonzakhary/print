		...
		<div class="invoice-box">
			<table cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<td>
									@if(count($order->address) > 0)
										{{ $order->address[0]->pivot->company_name !== "No Company" ? $order->address[0]->pivot->company_name : "" }}<br />
										{{ $order->orderedBy->profile->first_name }} {{ $order->orderedBy->profile->last_name }}<br />
				...
				<tr class="heading">
					...
					<td></td>
					<td>{{ (new \App\Plugins\Moneys())->setAmount($order->price->amount())->add($order->price->newFromTax($vat)->amount()) }}</td>
				</tr>
			</table>
		</div>
	</body>
