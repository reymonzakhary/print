  ...
  >
    ...
    <div>
      <table class="invoice-details" style="float: left; width: 60%">
        <tbody>
          <tr>
            <td style="width: 40%">{{ $t("Quotation number") }}:</td>
            <td style="width: 60%">001</td>
          </tr>
          <tr>
            <td style="width: 40%">{{ $t("Quotation date") }}:</td>
            <td style="width: 60%">2025-01-23 15:12:13</td>
          </tr>
          <tr>
            <td style="width: 40%">{{ $t("Expiration date") }}:</td>
            <td style="width: 60%">2025-02-07 15:12:13</td>
          </tr>
        </tbody>
      </table>
      <div style="float: right; width: 40%">
        <h1 class="invoice-title">{{ $t("Quotation") }}</h1>
      </div>
    ...
    >
      <table class="items">
        <thead>
          <tr>
            <th style="width: 42%">{{ $t("Product/Service") }}</th>
            <th style="width: 12%; text-align: right">{{ $t("VAT") }}</th>
            <th style="width: 17%; text-align: right">{{ $t("Unit price") }}</th>
        ...
        <tbody>
          <tr>
            <td>
              {{ $t("Product name") }} <br />
              <span style="font-size: 0.75em">{{ $t("Description of the product.") }}</span>
            </td>
            <td class="currency-cell"><span class="currency-symbol">%</span> 21,00</td>
          ...
          <tr class="border-b" style="border-color: #aaa; height: 2.5em">
            <td colspan="4">
              {{ $t("Shipping costs") }} <br />
              <span style="font-size: 0.75em">{{ $t("Over 21% VAT") }}</span>
            </td>
            <td class="currency-cell">
              <span class="currency-symbol">{{ getCurrencySymbol() }}</span
              >7,95
    ...
    <table class="totals">
      <tbody>
        ...
        <tr>
          <td style="width: 81%; text-align: right; padding-right: 1em">{{ $t("VAT 21%") }}</td>
          <td class="currency-cell" style="width: 19%">
            <span class="currency-symbol">{{ getCurrencySymbol() }}</span
            >22,67
          </td>
    ...
    <p class="footer-info">{{ $t("This quotation is valid for 15 days.") }}</p>
