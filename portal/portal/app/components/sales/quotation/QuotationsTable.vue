      ...
      <tr>
        ...
        <th class="w-12 xl:w-auto">
          <div class="flex items-center justify-end gap-1">
            <span
              v-if="
                (!props.isBin && permissions.includes('quotations-read')) ||
                (props.isBin && permissions.includes('quotations-trashed-read'))
              "
    ...
    <transition-group :name="`${props.activeView !== 'list' ? 'sales-list' : ''}`">
      ...
      >
        <tr>
          ...
          <td v-if="visibleColumns['nr']" class="font-mono font-bold text-gray-400">
            #{{ quotation.id }}
          </td>
          ...
          <td v-if="visibleColumns['status']">
            <SalesStatusIndicator :status="quotation.status.code" class="max-w-36" />
          </td>
          <td class="!overflow-visible">
            <div class="flex items-center justify-end gap-1">
              <UIButton
                v-if="
                  (!isBin && permissions.includes('quotations-read')) ||
                  (isBin && permissions.includes('quotations-trashed-read'))
                "
                ...
                :disabled="saving"
                @click.stop="navigateQuotation(quotation.id)"
              >
                {{ $t("open") }}
              </UIButton>
              <ItemMenu
                v-if="permissions.includes('quotations-delete') && !quotation.trashed"
            ...
            <tr class="!h-16 !overflow-visible bg-gray-100 shadow-inner dark:bg-gray-800">
              ...
              <td :colspan="visibleColumnsCount + 1" class="!overflow-visible">
                <OrderRows
                  ...
                  @on-cancel-item="
                    emit('cancel-item', { quotationId: quotation.id, itemId: item.id })
                  "
                />
              </td>
            </tr>
...
const visibleColumns = ref(
  JSON.parse(localStorage.getItem("visibleQuotationsColumns")) || defaultVisibleColumns,
);
...
watch(
  visibleColumns,
  (newVal) => {
    localStorage.setItem("visibleQuotationsColumns", JSON.stringify(newVal));
  },
  { deep: true },
);
...
function handleQuotationClick(quotationId, itemsLength) {
  ...
  if (props.activeView === "list") {
    ...
    navigateQuotation(quotationId);
  }
}

function navigateQuotation(quotationId) {
  if (saving.value) return;
  if (props.isBin) {
    return navigateTo({ path: `/quotations/${quotationId}`, query: { trashed: true } });
  }
  return navigateTo({ path: `/quotations/${quotationId}` });
}
...
function menuItemClicked(event, quotation) {
  switch (event) {
    case "delete":
      emit("onDeleteQuotation", quotation.id);
      break;
    default:
      break;
  }
...
const getHoverClass = (quotationId) => {
  ...
  if (props.activeView === "grid") {
    return {
      "!bg-gray-100 dark:!bg-gray-900 cursor-pointer": props.hoveredQuotation === quotationId,
    };
  } else {
    return { "hover:bg-gray-100 dark:hover:!bg-gray-800": true };
  }
};
</script>
