<template>
  ...
  >
    ...
    />
    <SalesAddTile
      v-if="!showOpenProduct && mayAddAnyProduct && isEditable && !isExternal"
      class="print:hidden"
      @on-finder-product="handleAddFinderProduct"
      @on-assortment-product="handleAddAssortmentProduct"
      @on-open-product="handleAddOpenProduct"
...
const { permissions, hasAnyPermissionGroup } = usePermissions();
...
const showZeroState = computed(
  () => !props.items.length && !showOpenProduct.value && criticalFlag.value === false,
);
const mayAddAnyProduct = computed(() =>
  hasAnyPermissionGroup([
    permissions.quotations.groups.addFinderProduct,
    permissions.quotations.groups.addOpenProduct,
    permissions.quotations.groups.addAssortmentProduct,
  ]),
);

const handleItemUpdated = (updatedItem) => emit("on-product-updated", updatedItem);
...
async function handleDuplicateItem(item) {
  try {
    ...
    const newItem = await quotationRepository.addItemToQuotation(
      props.salesId,
      newItemData.product,
    );
...
async function handleRemoveItem(itemId) {
  ...
  try {
    ...
    await quotationRepository.removeItemFromQuotation(props.salesId, itemId);
...
async function handleAddFinderProduct() {
  productFinderStore.activeQuotation = props.salesId;
...
async function handleAddAssortmentProduct() {
  ...
  store.commit("orders/set_active_order_type", "quotation");
  return navigateTo({
    path: "/assortment",
  });
}
...
async function handleSaveOpenProduct(product) {
  try {
    const data = await quotationRepository.addItemToQuotation(props.salesId, product);
