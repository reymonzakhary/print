  ...
  <article>
    ...
    >
        ...
        <div
          :class="
            me.tenant_id !== props.item.product.external_id
              ? `rounded border-2 border-green-200 p-3`
              : ''
          "
        >
          <p class="mb-2 truncate text-xs font-bold uppercase tracking-wide">
            ...
            {{ $t("producer") }}
            <span v-if="me.tenant_id !== props.item.product.external_id">
              <font-awesome-icon :icon="['fas', 'gear']" class="ml-3 mr-1" />
              {{ $t("has been produced by") }}
            </span>
          </p>
          <UIVSelect
            v-if="isEditable && props.item.editable && permissions.includes('orders-update')"
          ...
          <div v-else>
            ...
            <span
              v-if="
                me.tenant_id !== props.item.product.external_id &&
                props.item?.product?.order_ref_nr &&
                props.item?.product?.item_ref
              "
              class="text-sm font-bold"
              >{{ $t("in order number: ") }} {{ props.item.product.order_ref_nr }}
              {{ $t("with item number: ") }} {{ props.item.product.item_ref }}</span
            >
          </div>
          <div
            v-if="me.tenant_id !== props.item.product.external_id"
            class="mt-3 h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700"
          >
            <div
              v-if="props.item.status.code !== statusMap.DRAFT"
              class="relative h-full rounded-full"
              :class="[widthStatusBar]"
              :style="gradientBackground"
            >
              <div
                :class="`absolute right-0 h-2 w-2 animate-ping rounded-full`"
                :style="{ 'background-color': getStatusColor(props.item.status.code) }"
              />
            </div>
          </div>
        </div>
      ...
      <section class="text-sm">
        <details :open="isEditable || (isExternal && props.item.editable)">
          <summary
          ...
          >
            ...
            <ul :class="{ 'pb-2': key === '' && key !== 'undefined' && key !== null }">
              ...
              >
                ...
                <template v-if="isExternal && isEditable && props.item.editable">
                  <UITextArea
                    v-model="option.display_value"
                    type="text"
                    :name="option.display_key"
                    :disabled="isEditDisabled || option.dynamic"
                    @blur="handleInstantSave('options')"
                  />
                </template>
                <template v-else>
                  <span class="font-bold">{{ $display_name(option.value_display_name) ?? option.value }}</span>
                </template>
                <div v-if="option.value_dynamic" class="col-span-2 flex gap-3 text-gray-700">
                  <template v-if="option.value_dynamic_type === 'format' && option.value_height && option.value_width">
                    <span
      ...
      <section class="text-sm">
        <ul class="px-4">
          ...
          >
            ...
            <div
              v-if="
                isEditable &&
                (props.item.editable ||
                  props.item.status.code === statusMap.DRAFT ||
          ...
          >
            ...
            <div v-if="isEditable && props.item.editable && permissions.includes('orders-update')">
      ...
      <section class="px-4">
        ...
        <SalesNoteInput
          ...
          :disabled="isEditDisabled || !permissions.includes('orders-update')"
      ...
      <hr class="dark:border-gray-800" />
      <section class="px-4">
        <template v-if="isEditable && props.item.editable && permissions.includes('orders-update')">
          ...
          <UICurrencyInput
            :model-value="
              productDetails.calculated
                ? item.product.price.selling_price_ex
        ...
        <template v-else>
          <div class="flex">
            <h2 class="flex-1 text-sm font-bold uppercase tracking-wide">
              {{ $t("price") }}
              <small v-if="permissions.includes('orders-update')" class="normal-case text-gray-600">
                (
                {{
                  props.item.product.calculation_type === "full_calculation"
                    ? $t("full calculation")
            ...
            <VDropdown offset="4" placement="top">
              ...
              <template #popper>
                ...
                >
                  ...
                  <li class="flex justify-between py-1">
                    {{ $t("subtotal") }}:
                    <b class="ml-4 font-mono">{{ props.item.product.price.display_p }}</b>
                  </li>
                  <li class="flex justify-between border-b py-1 dark:border-gray-900">
        ...
        <template
          v-if="
            ...
            (props.item.editable ||
              props.item.status.code === statusMap.DRAFT ||
              props.item.status.code === statusMap.NEW ||
              props.item.status.code === statusMap.IN_PROGRESS) &&
            permissions.includes('orders-update')
          "
        >
      ...
      >
        <button
          class="relative px-2 py-3 text-xs transition-colors duration-75 hover:bg-theme-500 hover:text-themecontrast-700 dark:text-theme-100 dark:hover:bg-theme-700"
          :disabled="saving || productDetails.options.length === 0"
        ...
        <button
          class="relative px-2 py-3 text-xs text-red-400 transition-colors duration-75 hover:bg-theme-500 hover:text-themecontrast-700 dark:text-red-300 dark:hover:bg-theme-700"
          :disabled="saving"
...
const { isExternal, saving, multipleAddresses, isUsingTeamAddresses, isEditable, pickupAddresses } =
...
const gradientBackground = computed(() => {
  const currentIndex = chronologicalStatusList.indexOf(
    props?.item?.status?.name?.toUpperCase() ?? "",
  );
  if (currentIndex === -1) return ""; // fallback if invalid status

  const gradientColors = chronologicalStatusList
    .slice(1, currentIndex + 1) // from first to current status
    .map((status) => {
      const code = statusMap[status];
      // console.log(code, statusColorMap[code]);
      return getStatusColor(code);
    })
    .filter(Boolean); // remove undefined colors

  // Remove duplicates if you want a clean gradient
  const uniqueColors = [...new Set(gradientColors)];
  const hexColors = uniqueColors.map((color) => colorHexMap[color]).filter(Boolean);
...
const { statusMap, getStatusColor, getStatusShadow, colorHexMap, chronologicalStatusList } =
  useOrderStatus();

const widthStatusBar = computed(() => {
  const status = props.item.status.code;
  if (status === statusMap.IN_PROGRESS) return "w-1/6";
  if (status === statusMap.IN_PRODUCTION) return "w-2/6";
  if (status === statusMap.READY) return "w-3/6";
  if (status === statusMap.BEING_SHIPPED) return "w-4/6";
  if (status === statusMap.DELIVERED) return "w-full";
  return "w-0"; // fallback for unhandled statuses
});
...
const setProductDetails = async () => {
  ...
  productDetails.value.productionDays = props.item.product.price.dlv.days;
  ...
  productDetails.value.price = props.item.product.price?.selling_price_ex
    ...
    : 0;
  productDetails.value.vatPercentage = props.item.product.price.vat;
...
const api = useAPI();
const MakeProduceItem = async () => {
  const data = {
    items: [{ id: props.item.id }],
  };
  try {
    saving.value = true;
    disabledButton.value = true;
    await api.post(`orders/${props.orderId}/items/produce`, data);
    addToast({
      type: "success",
      message: $t("the item status has been successfully updated."),
    });
...
watch(
  ...
  async (newValue, oldValue) => {
    ...
    //     return;
    //   }
    //   pickupAddresses.value = await orderRepository.getPickupAddresses(mgrContext.id);
    //   return;
    // }
  },
);
...
const update = async (data) => {
  try {
    saving.value = true;
    const updatedItem = await orderRepository.updateItem({
      orderId: props.orderId,
      itemId: props.item.id,
      data: {
        ...data,
...
watch(
  ...
  (newValue, oldValue) => {
    ...
    if (updatedProperty.key === "deliveryAddress") {
      ...
      orderRepository.addAddressToItem({
        orderId: props.orderId,
        itemId: props.item.id,
        data: {
