<template>
  ...
  <div v-else class="flex h-full flex-col gap-4">
    ...
    <div class="sticky top-0 z-40 -mb-4 bg-gray-100 pb-4 dark:bg-gray-800 print:hidden">
      <PreviewLoader v-if="previewLoader" />
      <PDFViewer v-if="showPDFViewer" :url="pdfURL" @on-close="handleClosePDFViewer" />
      <OrderDetailsHeader
        :created-at="props.quotation.created_at"
        :status="props.quotation.status.code"
        class="mb-4"
        @on-show-history="showHistoryModal = true"
        @on-print-order="handlePrintOrder"
        @on-archive-order="emit('on-archive-order', $event)"
      />
    ...
    <SalesUserSelector
      ...
      :only-show="
        props.quotation.created_from === 'from-shop' || !permissions.includes('orders-update')
      "
    ...
    <div class="flex flex-col gap-4">
      <SalesPaymentMethod
        v-model="quotationDetails.paymentMethod"
        with-status
        :disabled="!!criticalFlag"
        :status="quotationDetails.paymentStatus"
        @update:status="quotationDetails.paymentStatus = $event"
      />
      <div
        v-if="
          isEditable &&
      ...
      >
        <div class="flex items-center gap-4 pb-2">
          ...
          <UISwitch
            ...
            @input="multipleAddresses = $event"
          />
        </div>
        <div
          v-if="!multipleAddresses"
          class="flex items-center gap-4 border-t border-gray-200 pt-2"
        >
          ...
          <UISwitch
            :disabled="quotationDetails.customerAddress?.id === null"
            class="mt-[2px]"
      ...
      <SalesDeliverySelector
        ...
        :address="quotationDetails.customerAddress || quotationDetails.invoiceAddress"
        :only-show="isExternal || noMembersAccess || !permissions.includes('orders-update')"
      ...
      <SalesReferenceInput
        ...
        :disabled="isHaveExternalItem || !permissions.includes('orders-update')"
        @blur="handleInstantSave"
      />
      <SalesNoteInput
        v-model="quotationDetails.note"
        :disabled="!permissions.includes('orders-update')"
        @blur="handleInstantSave"
      />
      <section>
        <OrderFiles
          ...
          :editable="isEditable && !isHaveExternalItem"
          :ext_connection="isExternal"
          :prop-driven="true"
          :upload-not-allowed="!permissions.includes('orders-media-create')"
          :delete-not-allowed="!permissions.includes('orders-media-delete')"
          :list-not-allowed="!permissions.includes('orders-media-list')"
          @on-file-uploaded="emit('quotation-updated', $event)"
          @on-remove-file="emit('quotation-updated', $event)"
        />
        <NuxtLink
          v-if="
            !isExternal &&
            permissions.includes('media-sources-access') &&
            (props.quotation?.attachments?.length > 0 ||
              props.quotation?.items.find((i) => i.attachments.length > 0))
          "
          :to="`/filemanager?path=orders/${Number(props.quotation.id)}`"
...
const emit = defineEmits(["on-quotation-updated", "on-archive-order", "on-print-order"]);
...
async function handleDetailsUpdated() {
  if (!JSON.parse(localStorage.getItem("orderSaveInfoShown"))) {
    addToast({
      type: "success",
    ...
    addToast({
      type: "info",
      message: $t("your orders are automatically saved when changes are made."),
    });
    localStorage.setItem("orderSaveInfoShown", "true");
  }
}
...
async function updateChooseableUsers() {
  const users = await fetchChooseableUsers();
  chooseableUsers.value = users;
  if (props.quotation.customer?.id && permissions.value.includes("members-list")) {
...
async function handleCreateAddress(address) {
  if (address.full_name === "") {
    address.full_name = quotationDetails.value.customer.name;
  }
  const userAddressEndpoint = `members/${quotationDetails.value.customer.id}/addresses`;
  ...
  try {
    const { data } = await api.post(apiUrl, address);
...
function setWarnings() {
  ...
  if (!multipleAddresses.value && !quotationDetails.value.customerAddress) {
    addWarning("customerAddress", $t("please select a delivery address"));
  } else {
    removeWarning("customerAddress");
  }
}
...
async function handlePrintOrder() {
  try {
    ...
    const response = await api.get(`/orders/${props.salesId}/render/pdf`, {
...
onMounted(async () => {
  leaving.value = false;
  if (isExternal.value || noMembersAccess.value) {
    quotationDetails.value.customer = {
      id: props.quotation.customer.id,
      name:
        props.quotation.customer.profile.first_name +
  ...
  await nextTick();
  quotationDetails.value.paymentStatus = props.quotation.payment_status ?? "0";
...
async function update(data) {
  try {
    saving.value = true;
    await orderRepository.updateOrder(props.salesId, data);
  } catch (err) {
    handleError(err);
  } finally {
    saving.value = false;
...
watch(
  ...
  (newValue, oldValue) => {
    ...
    if (updatedProperty.key === "customer") {
      ...
      updateWithSaver({ user_id: newValue.customer?.id });
    }

    if (updatedProperty.key === "paymentStatus") {
      updateWithSaver({ payment_status: newValue.paymentStatus });
    }
