...
#############################
class UnlinkedOptionsApi(Resource):
    @staticmethod
    def get():
        ...
        options = SupplierOption.objects.order_by(sort_dir + sort_by).aggregate([
            {
                "$match": {
                    "linked": None,  # Filter options where linked is None
        ...
        # Get total count
        count = options['count'][0]['count'] if options['count'] else 0
