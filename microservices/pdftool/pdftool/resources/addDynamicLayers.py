...
def get_layers(layers_directory, local_path):
    for group in layers_directory:
        ''' Download layers directory   '''
    #     os.makedirs(local_path)
        for layer_object in group:
            filename = layer_object.get('path').split('/')[-1]
            #check if layer exists ignore it
...
class AddDynamicLayersOnPdfs(Resource):
    ...
    def post(self):
        ...
        if os.environ.get('CONFIG') == 'Development':
            ...
            for grouped_layer_option in layers_options:
                template = templates_local_path + '/' + grouped_layer_option[0].get('template_name')
                add_layers(template, layers_local_path, grouped_layer_option, output_local_path)
