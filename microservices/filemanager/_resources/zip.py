...
class Zip(Resource):
    ...
    def post(self):
        ...
        no_conflict_zip_dir = f"noConflict-extracting-{time.time()}"
        if is_dir is None: is_dir = "1"
        ...
        if os.environ.get('CONFIG') == 'Development':
            ...
            try:
                ...
                if is_dir == "1":
                    ...
                    # create no conflict directory
                    os.makedirs(f"storage/{path}/{no_conflict_zip_dir}/")
                    path = disk+"/"+path
                    ...
                    down_process.wait()
                    shutil.make_archive(f"storage/{no_conflict_zip_dir}/{name}", 'zip', f'storage/{path}')
                    # preper upload directory command
                    dir = os.path.dirname(put_path)

                    upload_command = f"rclone sync \"storage/{no_conflict_zip_dir}/{name}.zip\" \"spaces-ams3:{os.environ.get('S3_BUCKET')}/{dir}\""
                    with subprocess.Popen(upload_command, shell=True, stdout=subprocess.PIPE) as up_processor:
                        (out, err) = up_processor.communicate()
                        # wait until upload complete
                    up_processor.wait()
                    # Upload a file to your Space
                    shutil.rmtree(f"storage/{no_conflict_zip_dir}")
                    shutil.rmtree('storage/' + path)
                    if os.path.exists('storage/' + put_path+".zip"):
                        os.remove('storage/' + put_path+".zip")
                    # return {"put_path" : put_path, "dir" : dir}
                    return {"message": "File has been Zipped successfully.", "status": 200}, 200
                elif (is_dir == "0"):
                    file = {}
                    files = data['elements'].split("|")
                    file_data = files[0].split('/')
                    file.update({'file_name': file_data[len(file_data) - 1]})
                    file_path =  file_data.pop()
                    file_path = "/".join(file_data)
                    file.update({'file_path' : file_path})
                    if path in file.get('file_path'):
                        file_dir_path = disk + '/' + file.get('file_path')
                    else :
                        file_dir_path = disk + '/' + path + '/' + file.get('file_path')

                    file_path = file_dir_path + '/' + file.get('file_name')
                    down_command = f"rclone sync \"spaces-ams3:{os.environ.get('S3_BUCKET')}/{file_path}\" \"storage/{file_dir_path}\""
                    with subprocess.Popen(down_command, shell=True, stdout=subprocess.PIPE) as down_process:
                        (out, err) = down_process.communicate()
                    down_process.wait()
                    new_file = file_dir_path + '/' + name

                    shutil.make_archive('storage/' + new_file, 'zip', 'storage/' + file_dir_path, file.get('file_name'))
                    upload_command = f"rclone sync \"storage/{new_file}.zip\" \"spaces-ams3:{os.environ.get('S3_BUCKET')}/{file_dir_path}\""
                    with subprocess.Popen(upload_command, shell=True, stdout=subprocess.PIPE) as up_processor:
                        (out, err) = up_processor.communicate()
                        # wait until upload complete
                    up_processor.wait()

                    shutil.rmtree(f"storage/{disk}/" + path)
                    return {"message": "File has been Zipped successfully.", "status": 200}, 200
            except:
