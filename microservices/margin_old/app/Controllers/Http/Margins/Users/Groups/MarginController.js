...
class MarginController {
    ...
    async store({ request, response }) {
        try {
             ...
             */
            const data = request.only([ 'reseller_id', 'user_group_id', 'margin' ])
            
            /**
             * create margin
             */
            const margin = await UserGroupMargin
                .updateOrCreate(
                    { "reseller_id": data.reseller_id, "user_group_id": data.user_group_id },
                    { "margin": data.margin }
                )
    ...
    async show({response, params})
    {
        try {
             ...
             */
            const data = await UserGroupMargin.where( 'reseller_id', params.reseller_id ).where('user_group_id', params.user_group_id).fetch()
    ...
    async showGeneral({response, params})
    {
        try {
             ...
             * define Margin array
             */
            const margin = await UserGroupMargin.where('reseller_id', params.reseller_id).where('user_group_id', params.user_group_id).first()
    ...
    async update({ params, request, response }) {
        try {
             ...
             * update margin
             */
            const margin = await UserGroupMargin.query()
                                          .where('_id', params.id)
