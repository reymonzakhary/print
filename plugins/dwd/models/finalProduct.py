...
class FinalProduct(db.Document):
    id = db.IntField()
    iso = db.StringField(required=True)
    supplier_category_id = db.ReferenceField(SupplierCategory)
    category_id = db.ReferenceField(Category)
    supplier_category_name = db.StringField(required=True)
    ...
    def __setattr__(self, key, value):
        super(FinalProduct, self).__setattr__(key, value)
        # if key == 'name':
