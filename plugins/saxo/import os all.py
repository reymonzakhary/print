...
class Saxo(Resource):
    ...
    def get(self, table=''):
        ...
        f = open('saxo/Stickers.6125.21012021 102919.saxoprint.co.uk_1.json', "r")
        ...
        for option in saxo_options.find():
            ...
            for valid_opt in saxo_products.find({"object."+box_name: option['name']}):
                for valid in valid_opt['object'].items():
                    # options_allt.update({valid[1]: valid[0]})
            ...
            for ex_valid in options.items():
                ...
                for ex_valid_opt in saxo_products.find(
                        {"object."+box_name: option['name'], "object."+ex_valid[1]: ex_valid[0]}):
                    for exclude_with_nd_opt in ex_valid_opt['object'].items():
                        ...
                        exclude_all.update(
                            {exclude_with_nd_opt[1]: exclude_with_nd_opt[0]})
                        exclude_all.pop(ex_valid[0], None)
                ...
                # return json.loads(dumps(options_alln))

                # therd selection
                options_and_exclude2 = []
                for ex_valid2 in exclude.items():
                    valid_with_nd_opt2 = {}
                    exclude2 = {}

                    # return json.loads(dumps(options_alln2))
                    # return json.loads(dumps({"object."+box_name: option['name'], "object."+ex_valid[1]: ex_valid[0], "object."+ex_valid2[1]: ex_valid2[0]}))

                    exclude2_all = {}
                    for ex_valid_opt2 in saxo_products.find(
                            {"object."+box_name: option['name'], "object."+ex_valid[1]: ex_valid[0], "object."+ex_valid2[1]: ex_valid2[0]}):

                        for exclude_with_nd_opt2 in ex_valid_opt2['object'].items():
                            exclude2.pop(exclude_with_nd_opt2[1], None)

                            if exclude_with_nd_opt2[1] == option['name'] or exclude_with_nd_opt2[1] == ex_valid[0] or exclude_with_nd_opt2[1] in valid_with_nd_opt:
                                pass
                            else:
                                exclude2.update(
                                    {exclude_with_nd_opt2[1]: exclude_with_nd_opt2[0]})
                                # valid_with_nd_opt2.update(
                                #     {exclude_with_nd_opt2[1]: exclude_with_nd_opt2[0]})
                            exclude2_all.update(
                                {exclude_with_nd_opt2[1]: exclude_with_nd_opt2[0]})
                    ...
                    exclude_all2 = {}
                    for x in options_all.items():
                        if x[1] == ex_valid[1] or x[1] == ex_valid2[1] or x[0] in exclude2_all:
                            pass
                        else:
                            exclude_all2.update({x[0]: x[1]})

                    if exclude_all2:
                        # return json.loads(dumps({"options": exclude_all2, "options and exclude": exclude_all, "options and exclude2": exclude2}))
                        options_and_exclude2.append(
                            {'option': ex_valid2[0], 'box': ex_valid2[1], 'exclude': exclude_all2})

                options_and_exclude.append(
                    {'option': ex_valid[0], 'box': ex_valid[1], 'include': options_and_exclude2})

            return json.loads(dumps({"options": options, "options and include": options_and_exclude}))

            saxo_options.update(
                {"name": option['name']}, {"$set": {"valid_options": options_and_exclude}})
