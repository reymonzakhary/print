...
class Database(object):
    def __init__(self):
        self.client = MongoClient(
            config['db']['url']+config['db']['user']+':'+config['db']['password']+'@'+config['db']['DB']+':'+config['db']['port'])  # configure db url
    ...
    def find(self, criteria, collection_name, projection=None, sort=None, limit=0, cursor=False):  # find all from db
        ...
        # found = self.db[collection_name].find({}, criteria).sort( { : 1 } ).limit( 5 )
        ...
        # found = list(found)
        res = []
        for i in found:  # to serialize object id need to convert string
            # if "_id" in found[i]:
            res.append(i)
