  ...
  <main>
    ...
    <header class="mb-8">
      ...
      <p class="mt-2 text-gray-600 dark:text-gray-400">
        Here's what's happening within the Prindustry Platforms today.
      </p>
    </header>
    ...
    <div class="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2 lg:grid-cols-4">
      ...
      >
        ...
        <div class="flex items-center mt-4">
          ...
          <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">matched</span>
        </div>
        <div class="flex items-center mt-4">
          <span class="text-sm font-medium text-blue-600 dark:text-blue-400">
            ↑ {{ unmatchedCategories.length }}
          </span>
          <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">unmatched</span>
        </div>
      </div>
...
// Get all standardization categories
const categories = ref([]);
const unmatchedCategories = ref([]);
const matchedCategories = ref([]);

onMounted(async () => {
  categories.value = await standardizationRepository.getCategories({ perPage: 99999 });
  unmatchedCategories.value = await standardizationRepository.getUnmatchedCategories();
  matchedCategories.value = await standardizationRepository.getMatchedCategories();
});
